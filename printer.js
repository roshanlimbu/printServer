...
class CustomPrinter {
  ...
  async printWithWindows(content, printerName = null) {
    return new Promise((resolve, reject) => {
      ...
      fs.writeFile(tempFile, content, this.encoding, (writeErr) => {
        if (writeErr) {
        ...
        if (printer) {
          ...
          command = `powershell "Get-Content '${tempFile}' | Out-Printer -Name '${printer}'"`;
        } else {
  ...
  async printToFile(content, filename = null) {
    return new Promise((resolve, reject) => {
      ...
      fs.writeFile(outputFile, content, this.encoding, (error) => {
        if (error) {
